...
class IsingModel():
    def __init__(self, N=50, h=0, T=1):
        """
    ...
    def glauber_spin_flip(self, i, j, p):
        ...
        deltaE = -2 * self.grid[i, j] * (self.sum_neighbours(i, j) - self.h)
    ...
    def calculate_total_energy(self):
        ...
        return 0.5 * np.sum(self.grid * neighbours) - self.h*np.sum(self.grid)
...
def main():
    ...
    parser.add_argument('h', type=float, help="The h.")
    ...
    model = IsingModel(N=args.N, T=args.T, h=args.h)
